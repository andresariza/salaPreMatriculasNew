<?php
namespace Sala\modelo;
defined('_EXEC') or die;
use \Sala\entidad\ModulosMenu;
 ...
 * @package model
 */
class Defecto implements \Sala\interfaces\Model{
    ...
    public function getRenderModulos($variables){
        ...
        if($option!="login" && $tmpl!="json"){
            ...
            $ModulosMenu = \Sala\entidad\ModulosMenu::getListModulosMenu($itemId);
            if(!empty($ModulosMenu)){
                foreach($ModulosMenu as $m){
                    ...
                    eval("\$Modelo = new \\Sala\\components\\".$moduloName."\\modelo\\".$modeloClass."();");
                    //d($Modelo);
