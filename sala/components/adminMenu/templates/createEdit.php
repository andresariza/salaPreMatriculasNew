...
<div class="panel">
    ...
    <form id="adminForm" name="adminForm" action="<?php echo HTTP_SITE;?>/index.php">
        <input type="hidden" name="option" value="adminMenu" />
        <input type="hidden" name="task" value="save" />
        <input type="hidden" name="tmpl" value="json" />
        <input type="hidden" name="idmenuopcion" value="<?php if(!empty($idmenuopcion)){  echo $MenuOpcion->getIdmenuopcion(); }?>" />
        <input type="hidden" name="linkmenuopcion" value="<?php echo (empty($linkmenuopcion))?"":$MenuOpcion->getLinkmenuopcion();?>" />
        <input type="hidden" name="nivelmenuopcion" value="<?php echo (empty($nivelmenuopcion))?"1":$MenuOpcion->getNivelmenuopcion();//1?>" />
        <input type="hidden" name="posicionmenuopcion" value="<?php echo (empty($posicionmenuopcion))?"02":$MenuOpcion->getPosicionmenuopcion();//02?>" />
        <input type="hidden" name="framedestinomenuopcion" value="<?php echo (empty($framedestinomenuopcion))?"contenidocentral":$MenuOpcion->getFramedestinomenuopcion();//contenidocentral?>" />
        <input type="hidden" name="transaccionmenuopcion" value="<?php echo (empty($transaccionmenuopcion))?mktime():$MenuOpcion->getTransaccionmenuopcion();?>" />
        <input type="hidden" name="codigotipomenuopcion" value="<?php echo (empty($codigotipomenuopcion))?"2":$MenuOpcion->getCodigotipomenuopcion();//2;?>" />
