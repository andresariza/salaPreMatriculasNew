...
abstract class Servicios{
    public static function getMateriasMatriculadas($codigoEstudiante, $codigoPeriodo){
        ...
        //use Sala\entidad\ViewMateriasMatriculadas;
        $db = Factory::createDbo();
        $where = " codigoestudiante = ".$db->qstr($codigoEstudiante)." AND codigoperiodo = ".$db->qstr($codigoPeriodo)." ";
        $listMateriasMatriculadas = \Sala\entidad\ViewMateriasMatriculadas::getList($where);
    ...
    public static function getListadoNotasPregrado($codigoEstudiante, $codigoPeriodo, $codigoCarrera){
        ...
        if(!empty($listMateriasMatriculadas)){ 
            ...
            //d($listadoMateria);
            $listadoCortes = \Sala\entidad\Corte::getList(" codigocarrera= ".$db->qstr($codigoCarrera) . " AND  codigoperiodo= ".$db->qstr($codigoPeriodo) . " AND codigomateria = 1 " );
            if(empty($listadoCortes)){
                $listadoCortes = \Sala\entidad\Corte::getList(" codigocarrera= ".$db->qstr($Carrera->getCodigoDiurno()) . " AND  codigoperiodo= ".$db->qstr($codigoPeriodo) . " AND codigomateria = 1 " );
            }
            ...
            foreach($listadoMaterias as $ObjMateria){
                ...
                $listadoCortesMateria = \Sala\entidad\Corte::getList(" codigocarrera = 1 AND  codigoperiodo= ".$db->qstr($codigoPeriodo) . " AND codigomateria = ".$db->qstr($ObjMateria->Materia->getCodigomateria()) . " " );

                if(empty($listadoCortesMateria)){
                    if($codigoCarreraCortes==$codigoCarreraMateria){
                        $listadoCortesMateria = $listadoCortes;
                    }else{
                        $listadoCortesMateria = \Sala\entidad\Corte::getList(" codigocarrera= ".$db->qstr($codigoCarreraMateria) . " AND  codigoperiodo= ".$db->qstr($codigoPeriodo) . " AND codigomateria = 1 " );
                    }
                }
    ...
    public static function getListadoNotasPosgrado($codigoEstudiante, $codigoPeriodo, $codigoCarrera){
        ...
        if(!empty($listMateriasMatriculadas)){ 
            ...
            foreach($listadoMaterias as $ObjMateria){
                ...
                $notas = \Sala\entidad\ViewHistoricoNotasEstudiante::getList($where);
    ...
    public static function getNotasMateriaCorte($Materia, $Corte, $Grupo, $codigoEstudiante, $codigoPeriodo){
        ...
        $notas = \Sala\entidad\DetalleNota::getList($where);
    ...
    public static function getPeriodosEstudiante($codigoEstudiante, $idEstudianteGeneral=null){
        ...
        $datos = \Sala\entidad\ViewHistoricoNotasEstudiante::getList($where, "codigoperiodo");
        ...
        if(!empty($codigosPeriodos)){
            $periodos = \Sala\entidad\Periodo::getList(" codigoperiodo IN (".implode(",",$codigosPeriodos).") ORDER BY codigoperiodo DESC");
        }
    ...
    public static function getPeriodoVigente(){
        ...
        $periodos = \Sala\entidad\Periodo::getList($where);
    ...
    public static function getPeriodoVirtualVigente($Carrera, $modalidadAcademica=false){
        ...
        $periodos = \Sala\Entidad\PeriodoVirtualCarrera::getList(implode(" AND ", $where));
    ...
    public static function getUserImage($Usuario){
        ...
        if(empty($userImageSession)){
            ...
            if(empty($linkimagen) && ($codigotipousuario == "600")){
                ...
                $listaEstudianteDocumento = \Sala\entidad\EstudianteDocumento::getList(" idestudiantegeneral = ".$db->qstr($Estudiante->getIdestudiantegeneral())." AND numerodocumento <> ".$db->qstr($Usuario->getNumerodocumento())." ORDER BY idestudiantedocumento DESC");
