/*
 * BTHEME ADMIN TEMPLATE JAVASCRIPT
 * ======================================================================
...
For more efficient, simply called "btheme.container".
...
!function ($) {
	...
	btheme.window.on('load', function(){
		...
		btheme.body.addClass('btheme-ready');
	});
...
!function ($) {
	...
	btheme.window.on('load', function() {
		...
		if(megadropdown.length) megadropdown.bthemeMega();
...
!function ($) {
	...
	btheme.window.one('load', function(){
		if (btheme.scrollTop.length && !btheme.isMobile) {
			var isVisible = true;
			var offsetTop = 250;

			btheme.window.scroll(function(){
				if (btheme.window.scrollTop() > offsetTop && !isVisible) {
					btheme.navbar.addClass('shadow');
					btheme.scrollTop.addClass('in');
					isVisible = true;
				}else if (btheme.window.scrollTop() < offsetTop && isVisible) {
					btheme.navbar.removeClass('shadow');
					btheme.scrollTop.removeClass('in');
					isVisible = false;
				}
			});

			btheme.scrollTop.on('click', function(e){
				e.preventDefault();

				btheme.bodyHtml.animate({
					scrollTop : 0
				}, 500);
			});
...
!function ($) {
	...
	methods = {
		'show' : function(el){
			...
			ovId = 'btheme-overlay-' + uID() + uID()+"-" + uID(),
			panelOv = $('<div id="'+ ovId +'" class="panel-overlay"></div>');

			el.prop('disabled', true).data('bthemeOverlay',ovId);
		...
		'hide': function(el){
			var target = $(el.attr('data-target'));
			var boxLoad = $('#'+ el.data('bthemeOverlay'));
	...
	$.fn.bthemeOverlay = function(method){
		if (methods[method]){
...
!function ($) {
	...
	$.bthemeNoty = function(options){
		...
		focusElement = function(pos){
			btheme.bodyHtml.animate({scrollTop: pos}, 300, function(){
				el.addClass('in');
			});
		},
		init = function(){
			if (opt.container === 'page') {
				if (!pageHolder) {
					...
					btheme.contentContainer.prepend(pageHolder);
				};
				...
				if (!floatContainer[opt.floating.position]) {
					...
					btheme.container.append(floatContainer[opt.floating.position]);
				}
...
!function ($) {
	...
	formElement = function(el){
		if (el.data('btheme-check')){
			return;
		}else{
			el.data('btheme-check', true);
		...
		changed = function(){
			if(input.type == 'radio' && $groupInput.length) {
				$groupInput.each(function(){
					var $gi = $(this);
					if ($gi.hasClass('active')) $gi.trigger('btheme.ch.unchecked');
			...
			if (input.checked) {
				el.addClass('active').trigger('btheme.ch.checked');
			}else{
				el.removeClass('active').trigger('btheme.ch.unchecked');
			}
		};
	...
	$.fn.bthemeCheck = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
				chk = methods[method].apply($(this).find('input'),Array.prototype.slice.call(arguments, 1));
	...
	btheme.document.ready(function() {
		allFormEl = $('.form-checkbox, .form-radio');
		if(allFormEl.length) allFormEl.bthemeCheck();
	});

	btheme.document.on('change', '.btn-file :file', function() {
		var input = $(this),
...
!function ($) {
		...
		$menulink.each(function(){
			...
			on('shown.bs.popover', function () {
				...
				offsetBottom 	= function(){
					if(btheme.container.hasClass('mainnav-fixed')){
				...
				if( offsetTop > offsetBottom){
					...
					if(!btheme.container.hasClass('navbar-fixed') && btheme.mainNav.hasClass('affix-top')) offsetBottom -= 50;
					if(popoverHeight > offsetBottom){
						if(btheme.container.hasClass('navbar-fixed') || btheme.mainNav.hasClass('affix-top')) offsetBottom -= (elHeight + 5);
			...
			.on('click', function(){
				if(!btheme.container.hasClass('mainnav-sm')) return;
	...
	updateSize = function(){
		...
		var sw = btheme.container.width(), currentScreen;
		...
		if (screenCat != currentScreen){
			...
			btheme.screenSize = currentScreen;

			if(btheme.screenSize == 'sm' && btheme.container.hasClass('mainnav-lg')){
				$.bthemeNav('collapse');
			}
		}
	},
	updateNav = function(e){
		...
		if (updateMethod == 'collapse' || btheme.container.hasClass('mainnav-sm') ) {
			btheme.container.removeClass('mainnav-in mainnav-out mainnav-lg');
			bindSmallNav();
		}
	...
	init = function(){
		if (!defaultSize) {
			defaultSize = {
				xs : 'mainnav-out',
				sm : btheme.mainNav.data('sm') || btheme.mainNav.data('all'),
				md : btheme.mainNav.data('md') || btheme.mainNav.data('all'),
				lg : btheme.mainNav.data('lg') || btheme.mainNav.data('all')
			}
	...
	methods = {
		'revealToggle' : function(){
			if (!btheme.container.hasClass('reveal')) btheme.container.addClass('reveal');
			btheme.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealIn' : function(){
			if (!btheme.container.hasClass('reveal')) btheme.container.addClass('reveal');
			btheme.container.addClass('mainnav-in').removeClass('mainnav-out mainnav-lg mainnav-sm');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'revealOut' : function(){
			if (!btheme.container.hasClass('reveal')) btheme.container.addClass('reveal');
			btheme.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'slideToggle' : function(){
			if (!btheme.container.hasClass('slide')) btheme.container.addClass('slide');
			btheme.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm')
			if(isSmallNav) unbindSmallNav();
			return;
		},
		...
		'pushToggle' : function(){
			btheme.container.toggleClass('mainnav-in mainnav-out').removeClass('mainnav-lg mainnav-sm');
			if (btheme.container.hasClass('mainnav-in mainnav-out')) btheme.container.removeClass('mainnav-in');
			//if (btheme.container.hasClass('mainnav-in')) //btheme.container.removeClass('aside-in');
			if(isSmallNav) unbindSmallNav();
			return;
		},
		'pushIn' : function(){
		...
		'colExpToggle' : function(){
			if (btheme.container.hasClass('mainnav-lg mainnav-sm')) btheme.container.removeClass('mainnav-lg');
			btheme.container.toggleClass('mainnav-lg mainnav-sm').removeClass('mainnav-in mainnav-out');
			return btheme.window.trigger('resize');
		},
		'collapse' : function(){
			btheme.container.addClass('mainnav-sm').removeClass('mainnav-lg mainnav-in mainnav-out');
			updateMethod = 'collapse';
			return btheme.window.trigger('resize');
		},
		'expand' : function(){
			btheme.container.removeClass('mainnav-sm mainnav-in mainnav-out').addClass('mainnav-lg');
			return btheme.window.trigger('resize');
		},
		'togglePosition' : function(){
			btheme.container.toggleClass('mainnav-fixed');
			btheme.mainNav.bthemeAffix('update');
		},
		'fixedPosition' : function(){
			btheme.container.addClass('mainnav-fixed');
			btheme.mainNav.bthemeAffix('update');
		},
		'staticPosition' : function(){
			btheme.container.removeClass('mainnav-fixed');
			btheme.mainNav.bthemeAffix('update');
		},
		'update' : updateNav,
	...
	$.bthemeNav = function(method,complete){
		if (methods[method]){
			if(method == 'colExpToggle' || method == 'expand' || method == 'collapse'){
				if(btheme.screenSize == 'xs' && method == 'collapse'){
					method = 'pushOut';
				}else if((btheme.screenSize == 'xs' || btheme.screenSize == 'sm') && (method=='colExpToggle' || method == 'expand') && btheme.container.hasClass('mainnav-sm')){
					method = 'pushIn';
				}
			}
	...
	$.fn.isOnScreen = function(){
		var viewport = {
			top : btheme.window.scrollTop(),
			left : btheme.window.scrollLeft()
		};
		viewport.right = viewport.left + btheme.window.width();
		viewport.bottom = viewport.top + btheme.window.height();
	...
	btheme.window.on('resizeEnd',updateNav).trigger('resize');
	...
	btheme.window.on('load', function() {
		...
		if (menu.length) {
			...
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			scrollbar = btheme.mainNav.find('.nano');
			if(scrollbar.length){
...
!function ($) {
	...
	var asideMethods = {
		'toggleHideShow' : function(){
			btheme.container.toggleClass('aside-in');
			btheme.window.trigger('resize');
			if(btheme.container.hasClass('aside-in')){
				toggleNav();
			}
		},
		'show' : function(){
			btheme.container.addClass('aside-in');
			btheme.window.trigger('resize');
			toggleNav();
		},
		'hide' : function(){
			btheme.container.removeClass('aside-in');
			btheme.window.trigger('resize');
		},
		'toggleAlign' : function(){
			btheme.container.toggleClass('aside-left');
			btheme.aside.bthemeAffix('update');
		},
		'alignLeft' : function(){
			btheme.container.addClass('aside-left');
			btheme.aside.bthemeAffix('update');
		},
		'alignRight' : function(){
			btheme.container.removeClass('aside-left');
			btheme.aside.bthemeAffix('update');
		},
		'togglePosition' : function(){
			btheme.container.toggleClass('aside-fixed');
			btheme.aside.bthemeAffix('update');
		},
		'fixedPosition' : function(){
			btheme.container.addClass('aside-fixed');
			btheme.aside.bthemeAffix('update');
		},
		'staticPosition' : function(){
			btheme.container.removeClass('aside-fixed');
			btheme.aside.bthemeAffix('update');
		},
		'toggleTheme' : function(){
			btheme.container.toggleClass('aside-bright');
		},
		'brightTheme' : function(){
			btheme.container.addClass('aside-bright');
		},
		'darkTheme' : function(){
			btheme.container.removeClass('aside-bright');
		}
	},
	toggleNav = function(){
		if(btheme.container.hasClass('mainnav-in') && btheme.screenSize != 'xs'){
			if(btheme.screenSize == 'sm'){
				$.bthemeNav('collapse');
			}else{
				btheme.container.removeClass('mainnav-in mainnav-lg mainnav-sm').addClass('mainnav-out');
			}
	...
	$.bthemeAside = function(method,complete){
	...
	btheme.window.on('load', function() {
		if(btheme.aside.length){
			// STYLEABLE SCROLLBARS
			// =================================================================
			// Require nanoScroller
			// http://jamesflorentino.github.io/nanoScrollerJS/
			// =================================================================
			btheme.aside.find('.nano').nanoScroller({
				preventPageScrolling : true,
				alwaysVisible: false
			});
...
!function ($) {
	...
	$.fn.bthemeLanguage = function(method){
		var chk = false;
		this.each(function(){
			if(methods[method]){
...
!function ($) {
	...
	$.fn.bthemeAffix = function(method){
		return this.each(function(){
			var el = $(this), className;

			if (typeof method === 'object' || !method){
				className = method.className;
				el.data('btheme.af.class', method.className);
			}else if (method == 'update') {
				className = el.data('btheme.af.class');
			}

			if (btheme.container.hasClass(className) && !btheme.container.hasClass('navbar-fixed') ) {
				el.affix({
					offset:{
					top:$('#navbar').outerHeight()
					}
				});
			}else if (!btheme.container.hasClass(className) || btheme.container.hasClass('navbar-fixed')) {
				btheme.window.off(el.attr('id') +'.affix');
	...
	btheme.window.on('load', function(){
		if (btheme.mainNav.length) {
			btheme.mainNav.bthemeAffix({className : 'mainnav-fixed'});
		}

		if (btheme.aside.length) {
			btheme.aside.bthemeAffix({className : 'aside-fixed'});
		}
	});
